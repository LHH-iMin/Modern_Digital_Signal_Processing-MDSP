function [Estimation_X0] = se3EKF_update(Estimation_X0, CameraMeasurementThis, Sigma_OB)
...
if ~isempty(IndexObservedAlreadyThis)
    ...
    for i = 1:NumberOfOldFeatureInThisStep
        ...
        %
        H(3*i-2:3*i, 1:3) = - orientation'*skew( fi );
...
if ~isempty(IndexObservedNew)
    ...
    for i = 1:NumberOfNewFeatureInThisStep
        ...
        KK( 6+3*NumberOfFeature+3*i-2:6+3*NumberOfFeature+3*i,1:3 ) = -skew(Estimation_X0.landmarks(1:3,NumberOfFeature+i))    ;

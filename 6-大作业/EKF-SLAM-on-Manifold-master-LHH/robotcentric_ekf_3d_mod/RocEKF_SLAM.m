function estimation_results = RocEKF_SLAM(data, NumberOfSteps)
% Robocentric-EKF SLAM 
...
for i = 0:NumberOfSteps
    ...
    if i ~= NumberOfSteps
        ...
        if m > 6
           ...
           [Estimation_X] = RocEKF_update(Estimation_X, CameraMeasurementThis, obsv_sigma );
        end
        
        estimation_results{i+1} = Estimation_X;
        
%        propagation using odometry info
        [Estimation_X] = RocEKF_propagate(Estimation_X, OdometryFromThis2Next, odom_sigma );

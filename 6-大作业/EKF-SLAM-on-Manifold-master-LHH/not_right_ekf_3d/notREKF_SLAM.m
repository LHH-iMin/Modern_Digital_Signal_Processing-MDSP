function estimation_results = notREKF_SLAM(data, NumberOfSteps)
% R-EKF SLAM 
...
for i = 0:NumberOfSteps
    ...
    if i==NumberOfSteps-1%运行到最后一个位置了
        a=1;
    end
    
    if i ~= NumberOfSteps%不是最后时刻
        %获得里程计数据
        OdometryFromThis2Next = data_matrix(IndexOfCurrentStepInDataMatrix(m-5):IndexOfCurrentStepInDataMatrix(m),1);
        if m > 6%m>6表明该时刻有地标观测数据
            %获得观测数据：带噪声的xyz观测数据，地标编号，不带噪声的xyz观测数据

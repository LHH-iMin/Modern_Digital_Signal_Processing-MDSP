function [Estimation_X0] = notREKF_update(Estimation_X0, CameraMeasurementThis, Sigma_OB)

NumberOfLandmarksObInThisStep = size(CameraMeasurementThis,1)/3;%该时刻观测到的地标个数

% initialise the IndexOfFeature if possible
if size(Estimation_X0.landmarks,2) > 0%判断是不是起始时刻
...
if ~isempty(IndexObservedNew)
    ...
    for i = 1:NumberOfNewFeatureInThisStep
        ...
        nf = CameraMeasurementThis( m2, 1 );%获得地标的xyz
...
%       nfnew = CameraMeasurementThis( m2, 3 );
        tempKK(3*i-2:3*i,3*i-2:3*i)=diag(nf.^2)*Sigma_OB^2;
    end
        Sigma   = blkdiag(Estimation_X0.cov,tempKK);%blkdiag将输入的子矩阵按照顺序排列在主对角线上，其余位置填充零元素。
